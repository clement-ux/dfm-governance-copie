...
/**
    ...
    @notice "Boost" refers to a bonus to claimable reward tokens that an account
            receives based on it's locked weight. An account with "max boost"
            is earning rewards at a multiplier `maxBoostMultiplier` compared
            ...
            * `maxBoostablePct` is set to 200. This means that during epoch 2, Alice
              can claim up to 20% (10% * 200%) of the rewards (100,000 tokens) with the
              maximum boost multiplier.
            * Once Alice's claims exceed 100,000 tokens, she enters the decay phase.
              `decayBoostPct` is set to 50, meaning Alice can claim up to 5% (10% * 50%) of
              the rewards (25,000 tokens) with a decaying boost.
            * Once Alice's claims exceed 125,000 tokens, any further claims that epoch
              receive no boost.
...
contract BoostCalculator is CoreOwnable, SystemStart {
    ...
    /**
        ...
        @param previousAmount Amount that was already claimed in this epoch
        @param totalEpochEmissions Total token emissions released this epoch
        @return adjustedAmount Amount of token received after applying boost
     */
    ...
    function _getAccountBoostData(
        ...
        if (paramChangeEpoch != 0 && paramChangeEpoch <= epoch) {
            ...
            (maxBoostable, fullDecay) = _getBoostable(totalEpochEmissions, lockPct, maxBoostablePct, decayBoostPct);
        }

        return (
            _getBoostedAmount(amount, previousAmount, lockPct, maxBoostMul, maxBoostable, fullDecay),
            maxBoostable,
            fullDecay
        );
    }
